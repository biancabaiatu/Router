...
/* Structure to be added in a queue containing the best route and a packet */
typedef struct {
	packet packet;
...
/* Sends a an ICMP message when the destination
 * of the received packet is reached
 */
void send_icmp(uint32_t daddr, uint32_t saddr, uint8_t *sha, uint8_t *dha, u_int8_t type, u_int8_t code, int interface);

/* Sends a an ICMP error message to the sender
 * stating either "Time exceeded" or "Destination unreachable".
 */
void send_icmp_error(uint32_t daddr, uint32_t saddr, uint8_t *sha, uint8_t *dha, u_int8_t type, u_int8_t code, int interface);

/* Iterates through the routing table in order
 * to find the address of the next hop. This function
...
/* Constructs a new packet with the payload
 * containing an arp request. This function
 * returns a packet structure.
 */
...
/* Calculates the checksum and decrements 
 * the time to leave using the RFC 1624 protocol
 */
void incremental_checksum(struct iphdr *ip_hdr);

int compare(const void *a, const void *b);

/* Calculates the longest prefix match using the 
 * binary search in the routing table
 */
